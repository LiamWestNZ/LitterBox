...
class TweetTestCase(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(username='liam', password='liamwest')
        self.userb = User.objects.create_user(username='testB', password='testtest2')
        Tweet.objects.create(content="my 1sr tweet", user=self.user)
        Tweet.objects.create(content="my 2nd tweet", user=self.user)
        Tweet.objects.create(content="my 3rd tweet", user=self.userb)
        self.currentCount = Tweet.objects.all().count()

    def test_tweet_created(self):
        tweet_obj = Tweet.objects.create(content="my 4th tweet", 
            user=self.user)
    ...
    def get_client(self):
        client = APIClient()
        client.login(username=self.user.username, password='liamwest')
        return client
